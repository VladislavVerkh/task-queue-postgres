...
| `task_queue_dead_letter`           | Архивация финализированной задачи                       | `TaskRetryService.retryOrFinalize()` -> `TaskDeadLetterRepository.deadLetterOwnedBy()`                               | если `dead-letter-enabled=true`: owner-checked copy задачи с причиной финализации и последней ошибкой                                                                                         |
...
| `task_queue_dead_letter`           | Retention cleanup                                       | `TaskDeadLetterService.deleteExpired()` -> `TaskDeadLetterRepository.deleteDeadLettersOlderThan()`                   | при `dead-letter-retention > 0`: batch-удаление DLQ-записей старше retention                                                                                                                   |
